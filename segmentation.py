...
def train_segmentation(model, device, train_loader,val_loader, optimizer, criterion, epochs=20):
    ...
    for epoch in range(epochs):
        ...
        for idx, sample in enumerate(train_loader):
            ...
            # IoU
            batch_train_iou = compute_iou(out_cut, target.data.cpu().numpy())
            ...
            if idx % 20 == 0:
                ...
                print('epoch {} batch {}  [{}/{}]\ttraining loss: {:1.4f} \tvalidation loss: {:1.4f}\t\tDice (val): {:.1%}\tIoU (val): {:.1%}'.format(epoch, idx, idx*len(data),
                        len(train_loader.dataset), loss.item(), val_loss, val_dice, val_iou))
    ...
    return val_loss_history, val_dice_history, val_iou_history
